...
func _use_dash(parameters: Dictionary) -> void:
	...
	if !parameters.target.combat_creature_movement_characteristics.is_using_movement_ability:
		...
		var timer = ability_utils._get_timer_or_create(timers_group_node, parameters, _finish_dash)
		timer.start()
...
func _use_dodge(parameters: Dictionary) -> void:
	...
	if !parameters.target.combat_creature_movement_characteristics.is_using_movement_ability:
		...
		var timer = ability_utils._get_timer_or_create(timers_group_node, parameters, _finish_dodge)
		timer.start()

func _movement_override_dodge(parameters) -> void:
	pass
